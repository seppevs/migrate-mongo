...
module.exports = {
  ...
  loadMigration(fileName) {
    const migration = require(path.join(resolveMigrationsDirPath(), fileName));
    const isAPIFormat = migration instanceof Migration;
    if (isAPIFormat) {
      return migration.convertToStandardFormat();
    }
    return migration;
  }
};
